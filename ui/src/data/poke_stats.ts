...
export function buildStatPercentile(gamedata: CompactGameData){
    ...
    gamedata.species.map((specie) => {
        ...
        // adding BST
        specie.baseStats[LEN_STATS_NO_BST] = BST
    ...
    gamedata.species.forEach((specie) => {
        specie.b_species_stats = specie.baseStats.map((x, i) => (sortedDataCum[i].indexOf(x) / speciesLen) * 100)
    })
}
